...
export default function RepoGraphPage({
      ...
      try {
        ...
        setError(
          caughtError instanceof Error
            ? caughtError.message
            : `Activity for ${repo} could not be loaded.`,
        );
      } finally {
    ...
    return (
      <GraphStateShell>
        <div role="status" className="text-center">
          ...
          <h1 className="mt-5 font-display text-xl font-medium">Mapping {repo}</h1>
          <p className="mt-1 text-sm font-semibold text-muted-foreground">
            Connecting this repository&apos;s activity…
          </p>
        </div>
      </GraphStateShell>
    );
      ...
      <GraphStateShell>
        <div role="alert" className="text-center">
          ...
          <div className="mt-6 flex flex-wrap justify-center gap-3">
            ...
            <Link href="/graph" className="pastel-control px-5 text-sm font-extrabold">
              All repositories
            </Link>
          </div>
        </div>
    ...
    return (
      <GraphStateShell>
        <div className="text-center">
          ...
          <h1 className="mt-5 font-display text-2xl font-medium">
            No indexed activity for {repo}
          </h1>
          <p className="mx-auto mt-2 max-w-md text-sm font-semibold leading-6 text-muted-foreground">
            This repository is not in the current index, or it has no imported graph
            records yet. Run ingestion and check again.
          </p>
          <div className="mt-6 flex flex-wrap justify-center gap-3">
            <button
              type="button"
  ...
  return (
    <div className="fixed inset-0 overflow-hidden bg-background">
      <header className="absolute top-3 left-3 z-20 max-w-[calc(100vw-1.5rem)] sm:top-4 sm:left-4">
        <div className="pastel-panel flex min-h-11 items-center gap-2 rounded-full px-2 py-1 shadow-[0_4px_0_var(--panel-shadow)]">
          <Link
            href="/graph"
            className="flex min-h-11 items-center gap-1.5 rounded-full px-2 text-sm font-extrabold text-muted-foreground hover:bg-secondary hover:text-foreground"
          >
            <svg
              aria-hidden="true"
              className="size-4"
          ...
          <span className="truncate pr-3 font-display text-sm font-medium text-[#3d7f70]">
            {repo}
          </span>
        </div>
      </header>
